...
7. 写真のダウンロード(REQ-046、FN-26)は、本人が `photos/{uid}/{photoId}.jpg` を読む既存の権限で行う(ルールは変えない)。並びと名前は、写真の記録の受信時刻(`receivedAt`)を使う(撮影時刻は保存していないため)。渡せるのは処理後の画像(位置情報を削除・縮小済み)だけで、元画像は残っていない。1枚ずつ・まとめて・全株の形は未決(要件 Q11)。まとめて渡すためにサーバー処理(圧縮ファイルを作る Cloud Function)が要るかは、Q11 が決まったときに決め、この設計書に足す。
...
| 18 | 写真のダウンロード | 新しい保存先・ルールは要らない見込み(`photos/` を本人が読める)。形は未決 | 要件 REQ-046・未決 Q11 |
