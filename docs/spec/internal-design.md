...
| `features/plants/` | `PlantFormScreen` | 株を追加・編集・削除する画面(SCR-05。追加=#17、編集・削除=#62)。株を渡すと編集。開いたときの入力の状態と比べて、戻るときの確認を決める |
...
| 6 | 株を追加・編集する画面 | 追加・編集・削除まで実装済み(#17・#62)。編集へのホームからの入口は、株の詳細(SCR-08)ができるまでの暫定(株をタップ)。撮影への遷移は未 | 外部設計 SCR-05 の「現状」に明記 |
