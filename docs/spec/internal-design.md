...
| `features/download/` | `DownloadScreen`(写真をダウンロード。SCR-13。#82)・`download_plan.dart`(対象の絞り込み・並び・ファイル名) | 設定・株の詳細・アカウント削除から開く。写真の保存は `PhotoSaver`(`data/photo_saver.dart`)経由 |
...
7. 写真のダウンロード(REQ-046、FN-26)は、本人が `photos/{uid}/{photoId}.jpg` を読む既存の権限で行う(ルールは変えない)。並びと名前は、写真の記録の受信時刻(`receivedAt`)を使う(撮影時刻は保存していないため)。渡せるのは処理後の画像(位置情報を削除・縮小済み)だけで、元画像は残っていない。形は決定済み(要件 Q11、外部設計 3.14):期間・株・写真で絞り、対象の写真を受信時刻の古い順に、1枚ずつ取得して端末の写真フォルダに保存する。圧縮ファイルは作らないので、**サーバー処理は増やさない**。写真の一覧は `…/photos` を購読して、アプリ側で期間・株・写真の絞り込みと、ファイル名(`株の名前_連番_受信日時.jpg`。連番は株ごと、名前の使えない文字は置き換え)を作る。削除済み・処理中の写真は、写真の記録がないか完了していないため、対象に入らない。写真フォルダへの保存は端末依存(Android・iOS の許可と保存の仕方が違う)。ライブラリの選定は、端末で確かめる段階(順13)で行い、この設計書を更新する。**実装(#82)**:`PhotoSaver`(`data/photo_saver.dart`。1枚を指定の名前で保存し、失敗したら `PhotoSaveException`(読み込めない/許可がない)を投げる差し替え口)と、動作確認用の `InMemoryPhotoSaver`(実際には保存しない。`isPlaceholder` が true のとき画面が注記を出す)。`photoSaverProvider` の既定は動作確認用。対象・並び・名前の決定は、画面から切り離した `buildDownloadPlan`(`features/download/download_plan.dart`)で、受信時刻の**日付**で期間を絞る・古い順(同じ時刻は作った順)・連番は株ごとに対象の中で1から・名前の重なりは `_2` で避ける。画面(`DownloadScreen`)は、1枚ずつ順に保存し、「やめる」は次の1枚の前で止める。件数が多くても止まらないよう、1枚ずつ順に保存し、途中でやめても保存済みは残す。
...
| 18 | 写真のダウンロード | 新しい保存先・ルール・サーバー処理は要らない(`photos/` を本人が読める。アプリが1枚ずつ保存)。画面と流れは実装済み(#82。保存先は動作確認用)。端末の写真フォルダへの保存は実機で確認 | 要件 REQ-046・Q11(決定済み) |
