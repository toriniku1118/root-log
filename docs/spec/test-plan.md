...
| 写真のダウンロード | 端末に保存できる。受信時刻の順に並ぶ。位置情報が入っていない | REQ-046 | 順10・順13 |
...
| 020 | — | 10章 | 単体:`image.test`(長辺の縮小・拡大しない) | 結合:無料/有料で画質が変わる(ステップ4) |
...
| 046 | SCR-08、SCR-11、FN-26 | 8章7、Q11 | ルール:既存の Storage テスト(「他人は非公開の写真を読めない」など)で、他人の写真を取れないことを確認済み | システム:株ごと・全株のダウンロード、受信時刻の順、削除した写真が入らない、位置情報が入っていない(結合:エミュレーター)。受入:実機で端末に保存できる |
...
| 写真のダウンロードの形を決めたとき(Q11) | 他人の写真をダウンロードできない(既存の Storage のテストで足りるか確認。サーバー処理を足すなら、本人以外が呼べないテストを追加) |
