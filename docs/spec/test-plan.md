...
| 巡回モードで1回にかかった時間 | 巡回の開始から完了まで(自分で計る) | 手間の確認 |
...
| 項目 | 確かめること | 要件 | いつ |
|---|---|---|---|
| カメラ | アプリ内カメラで撮影できる。ゴースト表示が重なる。許可がないときの案内 | REQ-012、014 | 順11 |
...
| 014 | SCR-06、FN-09 | — | なし(未実装) | システム:重ね表示。受入:実機 |
...
| 022 | FN-14 | Q4 | ルール:水やりの記録を本人が作れる | 単体:前回からの日数・「いつもの間隔」。システム:長押し・巡回・詳細から1タップで記録 |
...
| 026 | SCR-08、FN-16 | — | なし(未実装) | システム:再生 |
| 027 | SCR-07、FN-17 | Q6 | なし(未実装) | システム:置き場所の順・撮影→水やり→次へ・完了画面。受入:実機(撮影) |
