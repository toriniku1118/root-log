...
1. **要件と仕様を先に決める**。作業を始める前に、issue の本文に「要件(なぜ・何を実現するか)」「仕様(どう動くか・完了の条件)」を書く。決まっていない項目は、決めてから着手する。
2. **開発項目は必ず issue にする**。1つの issue が大きい・細かく分けたほうがよいときは、親 issue の下に sub-issue を作って分ける。issue のない作業はしない。
3. **作業は必ず worktree で行う**。`main` を直接編集しない。issue ごとに worktree とブランチを作る。
4. **PR を作って、マージまで進める**。PR の本文に `Closes #issue番号` を書く。**マージの承認は不要**。ただし、テストや型チェックが通っていることを確認してからマージする。
...
| パス | 内容 |
|---|---|
| `docs/dev-rules.md` / `.github/` | 開発の進め方の詳細、issue・PR のテンプレート |
