...
- **作り込む中核は3本(2026-09-26、ユーザー決定):「記録」「交換・売買」「交流」。** 記録も重要だが、作りはシンプルでよい(記録機能を作り込みすぎない。手間をかけるのは交換・売買と交流を効果的にする部分)。ゴースト表示・巡回モード・タイムラプスは後回し。ただし、タイムラプスなどを自分で作れるよう、自分の写真(素材)はすべてダウンロードできるようにする(REQ-046)。
